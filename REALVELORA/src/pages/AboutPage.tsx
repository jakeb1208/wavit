    ...
    <div className="min-h-screen bg-gray-300 pb-24 sm:pb-0">
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {content.features.map((f, i) => (
            <div key={i} className="bg-white rounded-2xl border-2 border-gray-200 shadow-md p-5 hover:border-blue-400 transition-colors">
              <div className="w-12 h-12 bg-blue-50 border-2 border-blue-200 rounded-2xl flex items-center justify-center mb-3">{ICONS[i % ICONS.length]}</div>
        ...
        <div className="bg-white rounded-2xl border-2 border-gray-200 shadow-md p-6 text-center">
          ...
          <Link to="/search" className="inline-flex items-center justify-center gap-2 px-6 py-3.5 bg-blue-600 border-2 border-blue-700 text-black font-bold text-sm hover:bg-blue-700 transition-colors shadow-sm">
            Find a Shop
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" /></svg>
          </Link>
        </div>
        <p className="text-center text-xs text-gray-500 font-medium pb-2">Built to make time work better. &copy;{new Date().getFullYear()} Wavit</p>
      </div>
    </div>
  );
}
