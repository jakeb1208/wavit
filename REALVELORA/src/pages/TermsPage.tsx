...
interface TermsSection { heading: string; body: string; }
interface TermsContent { last_updated: string; sections: TermsSection[]; }
    ...
    <div className="min-h-screen bg-[#f8f7ff] pb-16">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 pt-10">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-8">
          ...
          <p className="text-xs text-gray-400 mb-8">Last updated: {content.last_updated} · Draft</p>
          ...
          <div className="prose prose-sm max-w-none space-y-6 text-gray-700">
            {content.sections.map((section, i) => (
              <section key={i}>
                ...
                {section.body.split('\n\n').map((para, j) => (
                  <p key={j} className="text-sm leading-relaxed mt-2 first:mt-0">{para}</p>
                ))}
              </section>
            ))}
          </div>
