...
export default function RegisterPage() {
  ...
  const setCfField = (field: string) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => setCf(f => ({ ...f, [field]: e.target.value }));

  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => { const d = e.target.value.replace(/\D/g, '').slice(0, 10); setForm(f => ({ ...f, phone: d })); };
  const handlePinChange = (e: React.ChangeEvent<HTMLInputElement>) => { const d = e.target.value.replace(/\D/g, '').slice(0, 6); setForm(f => ({ ...f, adminPin: d })); };
  const handlePinConfirmChange = (e: React.ChangeEvent<HTMLInputElement>) => { const d = e.target.value.replace(/\D/g, '').slice(0, 6); setForm(f => ({ ...f, adminPinConfirm: d })); };

  const handleClinicPhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => { const d = e.target.value.replace(/\D/g, '').slice(0, 10); setCf(f => ({ ...f, phone: d })); };
  const handleClinicPinChange = (e: React.ChangeEvent<HTMLInputElement>) => { const d = e.target.value.replace(/\D/g, '').slice(0, 6); setCf(f => ({ ...f, adminPin: d })); };
  const handleClinicPinConfirmChange = (e: React.ChangeEvent<HTMLInputElement>) => { const d = e.target.value.replace(/\D/g, '').slice(0, 6); setCf(f => ({ ...f, adminPinConfirm: d })); };
  ...
  const handleClinicSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); setClinicStatus('loading'); setClinicError('');
    const digits = cf.phone.replace(/\D/g, '');
    if (digits.length !== 10) { setClinicError('Please enter a valid 10-digit US phone number.'); setClinicStatus('error'); return; }
    if (!cf.clinicName.trim()) { setClinicError('Clinic name is required.'); setClinicStatus('error'); return; }
    if (!cf.address.trim() || !cf.city.trim() || !cf.state) { setClinicError('Please fill in the full clinic address.'); setClinicStatus('error'); return; }
    if (cf.adminPin.length !== 6) { setClinicError('Please choose a 6-digit admin login PIN.'); setClinicStatus('error'); return; }
    if (cf.adminPin !== cf.adminPinConfirm) { setClinicError('Admin PINs do not match.'); setClinicStatus('error'); return; }
    try {
      const res = await fetch(`${API_BASE}/register`, {
        method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          businessName: cf.clinicName.trim(), ownerName: cf.ownerName.trim(),
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Submission failed');
      setClinicStatus('success');
    } catch (err: any) { setClinicError(err.message); setClinicStatus('error'); }
  };

  // ── Success screen ──
  const didSucceed = status === 'success' || clinicStatus === 'success';
  const successName = status === 'success' ? form.businessName : cf.clinicName;
  const successEmail = status === 'success' ? form.email : cf.email;

  if (didSucceed) {
    const card = (
      <div style={{ textAlign: 'center', background: GLASS, border: `1px solid ${BORDER}`, borderRadius: '24px', padding: '32px 28px', maxWidth: '380px', width: '100%', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)' }}>
        ...
        <p style={{ fontSize: '14px', color: 'rgba(148,163,184,0.7)', lineHeight: 1.65, marginBottom: '24px' }}>
          Thanks for registering <strong style={{ color: '#f0f4ff' }}>{successName}</strong>. We'll review your application and be in touch via <strong style={{ color: '#f0f4ff' }}>{successEmail}</strong>.
        </p>
  ...
  // ── Tab switcher ──
  const TabSwitcher = () => (
    <div style={{ display: 'flex', gap: '4px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.09)', borderRadius: '14px', padding: '4px', marginBottom: '20px' }}>
      <button type="button" onClick={() => setTab('standard')} style={{ flex: 1, padding: '10px 8px', borderRadius: '10px', border: 'none', cursor: 'pointer', background: tab === 'standard' ? 'rgba(59,130,246,0.85)' : 'transparent', color: tab === 'standard' ? '#fff' : 'rgba(148,163,184,0.65)', fontWeight: 700, fontSize: '13px', transition: 'all 0.18s', fontFamily: 'inherit' }}>
        For Businesses
      </button>
      <button type="button" onClick={() => setTab('clinic')} style={{ flex: 1, padding: '10px 8px', borderRadius: '10px', border: 'none', cursor: 'pointer', background: tab === 'clinic' ? 'rgba(59,130,246,0.85)' : 'transparent', color: tab === 'clinic' ? '#fff' : 'rgba(148,163,184,0.65)', fontWeight: 700, fontSize: '13px', transition: 'all 0.18s', fontFamily: 'inherit' }}>
        🏥 For Clinics
      </button>
    </div>
  );
      ...
      <div style={{ minHeight: '100vh', background: 'radial-gradient(ellipse 80% 30% at 50% 0%, rgba(30,58,138,0.28) 0%, #070b14 50%)', color: '#f0f4ff', padding: '20px 16px 40px' }}>
        ...
        {tab === 'standard' ? (
          <form onSubmit={handleSubmit}>
            ...
            <div style={sectionStyle}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ fontSize: '12px', color: 'rgba(148,163,184,0.55)', lineHeight: 1.5 }}>Let customers join from anywhere. <span style={{ color: '#fbbf24', fontWeight: 700 }}>Not recommended</span></p>
                </div>
            ...
            {status === 'error' && (<div style={{ background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: '14px', padding: '13px 16px', fontSize: '13px', color: 'rgba(248,113,113,0.9)', fontWeight: 600, marginBottom: '14px' }}>{errorMsg}</div>)}
            ...
            <p style={{ fontSize: '11px', color: 'rgba(148,163,184,0.4)', textAlign: 'center', lineHeight: 1.6 }}>
              ...
              <Link to="/terms" style={{ color: '#60a5fa', fontWeight: 700, textDecoration: 'none' }}>Terms of Service</Link>{' '}and{' '}
          ...
          <form onSubmit={handleClinicSubmit}>
            <div style={sectionStyle}>
              <p style={sectionTitleStyle}>Contact Info</p>
              <div style={{ marginBottom: '12px' }}><label style={labelStyle}>Owner / Manager Name *</label><input type="text" value={cf.ownerName} onChange={setCfField('ownerName')} placeholder="Dr. Jane Smith" required style={inputStyle} /></div>
              <div style={{ marginBottom: '12px' }}><label style={labelStyle}>Email *</label><input type="email" value={cf.email} onChange={setCfField('email')} placeholder="you@clinic.com" required style={inputStyle} /></div>
              <div style={{ marginBottom: '12px' }}>
                <label style={labelStyle}>Phone Number *</label>
                <div style={{ display: 'flex', alignItems: 'center', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '14px' }}>
                  <span style={{ padding: '13px 4px 13px 14px', fontSize: '14px', fontWeight: 700, color: 'rgba(148,163,184,0.6)', flexShrink: 0 }}>+1</span>
                  <input type="tel" value={formatPhoneDisplay(cf.phone)} onChange={handleClinicPhoneChange} placeholder="(555) 000-0000" required style={{ flex: 1, padding: '13px 14px', background: 'transparent', border: 'none', color: '#f0f4ff', fontSize: '14px', fontWeight: 500, outline: 'none' }} />
                </div>
              </div>
              <div><label style={labelStyle}>Typical # of Doctors Working *</label><input type="number" min="1" max="50" value={cf.numDoctors} onChange={setCfField('numDoctors')} required style={inputStyle} /></div>
            </div>
            <div style={sectionStyle}>
              <p style={sectionTitleStyle}>Clinic Details</p>
              <div style={{ marginBottom: '12px' }}><label style={labelStyle}>Clinic Name *</label><input type="text" value={cf.clinicName} onChange={setCfField('clinicName')} placeholder="e.g. Sunrise Family Clinic" required style={inputStyle} /></div>
              <div style={{ marginBottom: '12px' }}><label style={labelStyle}>Street Address *</label><input type="text" value={cf.address} onChange={setCfField('address')} placeholder="123 Main St" required style={inputStyle} /></div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '12px' }}>
                <div><label style={labelStyle}>City *</label><input type="text" value={cf.city} onChange={setCfField('city')} placeholder="City" required style={inputStyle} /></div>
                <div>
                  <label style={labelStyle}>State *</label>
                  <select value={cf.state} onChange={setCfField('state')} required style={{ ...inputStyle, appearance: 'none' as const, WebkitAppearance: 'none' as const }}>
                    <option value="" style={{ background: '#0f1629' }}>--</option>
                    {US_STATES.map(s => <option key={s} value={s} style={{ background: '#0f1629' }}>{s}</option>)}
                  </select>
                </div>
              </div>
            ...
            <div style={sectionStyle}>
              ...
              <div style={{ marginBottom: '12px' }}>
                <label style={labelStyle}>Create 6-digit Admin PIN *</label>
                <input type="password" inputMode="numeric" autoComplete="new-password" value={cf.adminPin} onChange={handleClinicPinChange} placeholder="Enter 6 digits" required maxLength={6} style={{ ...inputStyle, textAlign: 'center', letterSpacing: '0.35em', fontSize: '20px', fontWeight: 900 }} />
              </div>
              <div>
                <label style={labelStyle}>Confirm 6-digit Admin PIN *</label>
                <input type="password" inputMode="numeric" autoComplete="new-password" value={cf.adminPinConfirm} onChange={handleClinicPinConfirmChange} placeholder="Re-enter 6 digits" required maxLength={6} style={{ ...inputStyle, textAlign: 'center', letterSpacing: '0.35em', fontSize: '20px', fontWeight: 900 }} />
                {cf.adminPinConfirm.length === 6 && cf.adminPin !== cf.adminPinConfirm && (<p style={{ fontSize: '12px', color: '#f87171', fontWeight: 700, marginTop: '6px' }}>PINs do not match.</p>)}
                <p style={{ fontSize: '11px', color: 'rgba(148,163,184,0.45)', marginTop: '8px', lineHeight: 1.5 }}>Used to access your clinic's admin dashboard after approval.</p>
              </div>
            </div>
            {clinicStatus === 'error' && (<div style={{ background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: '14px', padding: '13px 16px', fontSize: '13px', color: 'rgba(248,113,113,0.9)', fontWeight: 600, marginBottom: '14px' }}>{clinicError}</div>)}
            <button type="submit" disabled={clinicStatus === 'loading'} style={{ width: '100%', padding: '15px', borderRadius: '16px', border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg, #3b82f6 0%, #6366f1 100%)', color: '#fff', fontSize: '15px', fontWeight: 800, letterSpacing: '-0.2px', boxShadow: '0 4px 20px rgba(59,130,246,0.4)', opacity: clinicStatus === 'loading' ? 0.6 : 1, boxSizing: 'border-box', marginBottom: '12px' }}>
              {clinicStatus === 'loading' ? 'Submitting…' : 'Submit Clinic Application'}
            </button>
            <p style={{ fontSize: '11px', color: 'rgba(148,163,184,0.4)', textAlign: 'center', lineHeight: 1.6 }}>
              By submitting you agree to our{' '}
              <Link to="/terms" style={{ color: '#60a5fa', fontWeight: 700, textDecoration: 'none' }}>Terms of Service</Link>{' '}and{' '}
    ...
    <div style={{ minHeight: '100vh', background: 'radial-gradient(ellipse 80% 30% at 50% 0%, rgba(30,58,138,0.28) 0%, #070b14 50%)', color: '#f0f4ff', paddingBottom: '40px' }}>
      <div className="max-w-xl mx-auto px-4 sm:px-6 pt-6 sm:pt-10">
        <div style={{ marginBottom: '24px' }}>
          ...
          <p style={{ fontSize: '13px', color: 'rgba(148,163,184,0.65)' }}>Get Wavit for your business — we'll review your application and reach out within 1–2 days.</p>
        </div>
        ...
        {tab === 'standard' ? (
          ...
          /* ── Clinic form (web) ── */
          <form onSubmit={handleClinicSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <div style={sectionStyle}>
              <p style={sectionTitleStyle}>Contact Info</p>
              <div style={{ marginBottom: '12px' }}><label style={labelStyle}>Owner / Manager Name *</label><input type="text" value={cf.ownerName} onChange={setCfField('ownerName')} placeholder="Dr. Jane Smith" required style={inputStyle} /></div>
              <div style={{ marginBottom: '12px' }}><label style={labelStyle}>Email *</label><input type="email" value={cf.email} onChange={setCfField('email')} placeholder="you@clinic.com" required style={inputStyle} /></div>
            ...
            <div style={sectionStyle}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '12px' }}>
                ...
                <div>
                  <label style={labelStyle}>State *</label>
                  <select value={cf.state} onChange={setCfField('state')} required style={selectStyle}>
                    <option value="" style={{ background: '#0f1629' }}>--</option>
                    {US_STATES.map(s => <option key={s} value={s} style={{ background: '#0f1629' }}>{s}</option>)}
                  </select>
                </div>
            ...
            <div style={sectionStyle}>
              <p style={sectionTitleStyle}>Admin PIN</p>
              <div style={{ marginBottom: '12px' }}><label style={labelStyle}>Create 6-digit Admin PIN *</label><input type="password" inputMode="numeric" autoComplete="new-password" value={cf.adminPin} onChange={handleClinicPinChange} placeholder="Enter 6 digits" required maxLength={6} style={{ ...inputStyle, textAlign: 'center', letterSpacing: '0.35em', fontSize: '20px', fontWeight: 900 }} /></div>
              <div>
                <label style={labelStyle}>Confirm 6-digit Admin PIN *</label>
                <input type="password" inputMode="numeric" autoComplete="new-password" value={cf.adminPinConfirm} onChange={handleClinicPinConfirmChange} placeholder="Re-enter 6 digits" required maxLength={6} style={{ ...inputStyle, textAlign: 'center', letterSpacing: '0.35em', fontSize: '20px', fontWeight: 900 }} />
                {cf.adminPinConfirm.length === 6 && cf.adminPin !== cf.adminPinConfirm && (<p style={{ fontSize: '12px', color: '#f87171', fontWeight: 700, marginTop: '6px' }}>PINs do not match.</p>)}
            ...
            {clinicStatus === 'error' && (<div style={{ background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: '14px', padding: '13px 16px', fontSize: '13px', color: 'rgba(248,113,113,0.9)', fontWeight: 600 }}>{clinicError}</div>)}
            <button type="submit" disabled={clinicStatus === 'loading'} style={{ width: '100%', padding: '15px', borderRadius: '16px', border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg, #3b82f6 0%, #6366f1 100%)', color: '#fff', fontSize: '15px', fontWeight: 800, letterSpacing: '-0.2px', boxShadow: '0 4px 20px rgba(59,130,246,0.4)', opacity: clinicStatus === 'loading' ? 0.6 : 1, boxSizing: 'border-box' as const }}>{clinicStatus === 'loading' ? 'Submitting…' : 'Submit Clinic Application'}</button>
            <p style={{ fontSize: '11px', color: 'rgba(148,163,184,0.4)', textAlign: 'center', lineHeight: 1.6 }}>We review all clinic applications manually. You'll hear from us within 1–2 business days.</p>
            <p style={{ fontSize: '11px', color: 'rgba(148,163,184,0.35)', textAlign: 'center', lineHeight: 1.6, paddingBottom: '8px' }}>By submitting you agree to our{' '}<Link to="/terms" style={{ color: '#60a5fa', fontWeight: 700, textDecoration: 'none' }}>Terms of Service</Link>{' '}and{' '}<Link to="/privacy" style={{ color: '#60a5fa', fontWeight: 700, textDecoration: 'none' }}>Privacy Policy</Link>.</p>
          </form>
        )}
      </div>
